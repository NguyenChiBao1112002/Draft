...
if(actualErrMess.equals(expectedErrMess)) {
	KeywordUtil.logInfo("The error message was correct as expected to be \"" + expectedErrMess + "\"")
	CustomKeywords.'common.UtilKeyword.takeScreenShot'("invalid_phone_err_mess")
}else {
	KeywordUtil.markFailedAndStop("The error message is not as expected \"" + expectedErrMess + "\"")
}	
...
if(WebUI.waitForElementVisible(findTestObject('Object Repository/common/span/id_span_class', [('id'): 'info-tab', ('class'): 'close-popup']), 5, FailureHandling.OPTIONAL)) {
	WebUI.click(findTestObject('Object Repository/common/span/id_span_class', [('id'): 'info-tab', ('class'): 'close-popup']))
}
